...
public class MediaNotification extends BroadcastReceiver {
	...
	public static final String ACTION_ADD="com.smash.up.add";
    ...
    public MediaNotification(MusicService service) {
        ...
        mIntents.put(R.drawable.pause, PendingIntent.getBroadcast(mService, 100,
                new Intent(ACTION_PAUSE).setPackage(pkg), PendingIntent.FLAG_CANCEL_CURRENT));
        mIntents.put(R.drawable.play, PendingIntent.getBroadcast(mService, 100,
                new Intent(ACTION_PLAY).setPackage(pkg), PendingIntent.FLAG_CANCEL_CURRENT));
        mIntents.put(R.drawable.skip_previous, PendingIntent.getBroadcast(mService, 100,
                new Intent(ACTION_PREV).setPackage(pkg), PendingIntent.FLAG_CANCEL_CURRENT));
        mIntents.put(R.drawable.skip_next, PendingIntent.getBroadcast(mService, 100,
        ...
        if ((mPlaybackState.getActions() & PlaybackState.ACTION_SKIP_TO_PREVIOUS) != 0) {
            mNotificationBuilder
                    .addAction(R.drawable.skip_previous,
                            "Previous",
                            mIntents.get(R.drawable.skip_previous));
            playPauseActionIndex = 1;
        }
        mNotificationBuilder.addAction(mPlayPauseAction);
        // If skip to next action is enabled
        if ((mPlaybackState.getActions() & PlaybackState.ACTION_SKIP_TO_NEXT) != 0) {
            mNotificationBuilder.addAction(R.drawable.skip_next,
                    "Next",
                    mIntents.get(R.drawable.skip_next));
        }
		Bitmap bitmap = mMetadata.getBitmap(MediaMetadataCompat.METADATA_KEY_ALBUM_ART);
		if(bitmap==null){
			bitmap=BitmapFactory.decodeResource(mService.getResources(),R.drawable.some_smash);
		}
