...
OnCompletionListener, OnErrorListener, AudioManager.OnAudioFocusChangeListener
{
	...
	public static final String ACTION_START= "com.smash.smash.start";

    private static final String TAG = "MusicService";
    // Action to thumbs up a media item
	boolean pauseOnStart =false;
    ...
    // "Now playing" queue:u
 	public static BottomPlayerPlugin mBottomPlayer;
	...
	public Musica getPlayingMusica()
	{
		return mPlayingMusicas.get(mCurrentPlayingPos);
	}
    ...
    public void onCreate()
	{
		...
		mBottomPlayer = new BottomPlayerPlugin(this);
		...
		try
		{
			mBottomPlayer.initListener(mSession.getSessionToken());
		}
	...
	{
		LogHelper.w(TAG, "onStart");
		if(intent.getAction()!=null)
			switch(intent.getAction()){
				case ACTION_START:
					mServiceStarted=true;
					repeatState=MusicMasterHandler.getRepeatState(this);
					unshuffableList = MusicMasterHandler.getToPlay(this);
					mCurrentPlayingPos = MusicMasterHandler.getToPlayPosition(this);
					mPlayingMusicas = MusicMasterHandler.getToPlay(this);
					pauseOnStart= true;
					handlePlayRequest();
					break;
				case ACTION_PLAY:
	...
	public int toggleRepeatState(){
		repeatState=MusicMasterHandler.toggleRepeatState(this);
	...
	public boolean shuffle(){
		...
		boolean shuffleState=MusicMasterHandler.toggleShuffling(this);
		Musica m=mPlayingMusicas.get(mCurrentPlayingPos);
		if(shuffleState){
			Collections.shuffle(MusicMasterHandler.getToPlay(this));
			mPlayingMusicas=MusicMasterHandler.getToPlay(this);
		}
		else{
			mPlayingMusicas=unshuffableList;
			MusicMasterHandler.setToPlay(this,unshuffableList, false);
		}
    ...
    private void handlePlayRequest()
	{
		...
		MusicMasterHandler.storeMusicaIndex(this, mCurrentPlayingPos);
        mDelayedStopHandler.removeCallbacksAndMessages(null);
        if (!mServiceStarted)
		{
    ...
    private void configMediaPlayerState()
	{
		...
		{  // we have audio focus:
            ...
            if (mPlayOnFocusGain)
			{
                if (!mMediaPlayer.isPlaying() && !pauseOnStart)
				{
    ...
    void playCurrentSong()
	{
		...
		String path=mPlayingMusicas.get(mCurrentPlayingPos).getMusicaKey();
		updateMetadata();
		try
		{
			Musica musica = mPlayingMusicas.get(mCurrentPlayingPos);
			if(!MusicMasterHandler.isOffline(this, musica))mMediaPlayer.setDataSource(this,Uri.parse(musica.getMusicaUri()));
			else mMediaPlayer.setDataSource(new File(getFilesDir(), musica.key).getAbsolutePath());
		...
		catch (Exception e)
		{
			...
			Musica.getMusicasStorage().child(path).getDownloadUrl().addOnSuccessListener(
				new OnSuccessListener<Uri>(){
					...
					{
						Musica m=mPlayingMusicas.get(mCurrentPlayingPos);
						m.setMusicaUri(uri.toString());
						Musica.getMainRef().child(m.key).child("musicaUri").setValue(uri.toString());
						Toast.makeText(MusicService.this,"Alterado",2000).show();
					}
				});
		}

		/*Commons.mDefaultStorageRef.child("musicas").child(path).getDownloadUrl().addOnSuccessListener(
		 new OnSuccessListener<Uri>(){
    ...
    private void updateMetadata()
	{
		...
		final MediaMetadataCompat.Builder metadataBuilder=new MediaMetadataCompat.Builder()
			...
			.putLong(MediaMetadataCompat.METADATA_KEY_DURATION,m.getDuration())
			.putString(MediaMetadataCompat.METADATA_KEY_ARTIST, m.getArtistaNome())
			.putString(MediaMetadataCompat.METADATA_KEY_ALBUM_ART_URI,m.getAlbumKey())
			.putString(MediaMetadataCompat.METADATA_KEY_ALBUM, m.getAlbumNome());
		mSession.setMetadata(metadataBuilder.build());
		final Target target = new Target() {
			@Override
			public void onBitmapLoaded(Bitmap bitmap, Picasso.LoadedFrom from)
			{
				Log.w(TAG,"Bitmap loaded");
			...
			public void onBitmapFailed(Drawable errorDrawable)
			{
				...
				Bitmap bitmap=BitmapFactory.decodeResource(getResources(),R.drawable.ic_album);
				metadataBuilder.putBitmap(MediaMetadataCompat.METADATA_KEY_ALBUM_ART,bitmap);
				metadataBuilder.putBitmap(MediaMetadataCompat.METADATA_KEY_ART,bitmap);
				mSession.setMetadata(metadataBuilder.build());
			}
		...
		Album.getAlbumArt(m.getAlbumKey(), new Album.OnGetAlbumArtListener(){

				@Override
				public void onResult(Uri i)
				{
					if(i!=null)Picasso.with(MusicService.this).load(i).into(target);
				}
			});
    ...
    private void setCustomAction(PlaybackStateCompat.Builder stateBuilder)
	{
		...
		{
			int favoriteIcon = R.drawable.ic_bookmark_plus;
            if (User.isFavorite(mPlayingMusicas.get(mCurrentPlayingPos)))
			{
                favoriteIcon = R.drawable.ic_bookmark_check;
            }
